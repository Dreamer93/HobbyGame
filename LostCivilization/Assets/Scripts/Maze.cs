...
public class Maze : MonoBehaviour {
	...
	//Assign walls to the cells
		for (int cellproces = 0; cellproces < cells.Length; cellproces++) {
			cells [cellproces] = new Cell ();
			cells [cellproces].east = allWalls [eastWestProces];
			cells [cellproces].south = allWalls [childProcess + (xSize + 1) * ySize];
			if (termCount == xSize) {
				eastWestProces += 2;
				termCount = 0;
			} else {
				...
				termCount++;
				childProcess++;
				cells [cellproces].west = allWalls [eastWestProces];
				cells [cellproces].north = allWalls[(childProcess+(xSize+1)*ySize)+xSize-1];
	...
	void CreateMaze () {
		...
		int location = 0;
		if (side == 1 || side == 4) {
			location = Random.Range (0, xSize);
		} else {
			location = Random.Range (1, ySize+1);
		}
		switch (side) {
		case 1:
			Destroy(cells[xSize*(ySize-1)+location].north);
			break;
		case 2:
			//TODO: WHY=!=!=!
			break;
		case 3:
			Destroy(cells[xSize*location-1].west);
			break;
		case 4:
			Destroy(cells[location].south);
			break;
				}
